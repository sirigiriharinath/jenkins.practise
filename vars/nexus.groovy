def nexus(COMPONENT) {
  ...
  command = "curl -f -v -u admin:admin --upload-file ${FILENAME} http://10.1.2.210:8081/repository/${COMPONENT}/${FILENAME}"
...
def make_artifacts(APP_TYPE, COMPONENT) {
  ...
  if(APP_TYPE == "NGINX") {
    ...
    command = "cp target/*.jar ${COMPONENT}.jar && zip -r ${FILENAME} ${COMPONENT}."
    def execute_com=sh(returnStdout: true, script: command)
    print execute_com
